...
def translateBook(name):
	
	f = open('patched/' + name + '.usfm')
	#f = open('seb/Mark.usfm')
...
#
books = os.listdir('patched')
books = [b[:-5] for b in books if b[-5:] == '.usfm']
preface = unicode(open('preface/preface.tex').read(), 'utf-8').strip()
bookTex = preface
for book in books:
	bookTex = bookTex + translateBook(book)
saveAll(bookTex)